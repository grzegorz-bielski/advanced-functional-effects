      ...
      test("apply") {
        ...
        test("fromQueue") {
          for {
            ...
            _      <- stream.foreach(value => ref.update(_ + value)) // will take from q until it is closed
            v      <- ref.get
  ...
  def spec =
    suite("RunningStreams") {
      ...
      test("runHead") {
        ...
        test("foldZIO") {
          val expected =
            SortedMap("What is your name?" -> "Sherlock Holmes", "What is your age?" -> "42")
      ...
      test("flatMap") {
        ...
        test("mapAccum") {
          ...
          def aggregate(stream: Stream[Nothing, String]): Stream[Nothing, (String, Int)] =
            stream.mapAccum[Map[String, Int], (String, Int)](Map.empty) { case (map, str) => 
              map.get(str) match {
                case Some(count) => (map + (str -> (count + 1)), (str -> (count + 1)))
  ...
  def spec =
    suite("BasicError") {
      ...
      test("fail") {
        ...
        test("catchAll") {
          for {
            value <- (ZStream.fail("Uh oh!").catchAll(str => ZStream(str))).runCollect
