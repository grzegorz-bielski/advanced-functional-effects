/** STREAMS
  ...
  *
  * Although FS2 and other streaming libraries can be used with ZIO, ZIO Streams
  * offers a compelling package in a small surface area and is considered
  * extremely vital to the future of the ZIO ecosystem.
  */
...
/** ZIO Streams can be constructed from a huge number of other data types,
  * including iterables, ZIO effects, queues, hubs, input streams, and much,
  * much more.
  *
      ...
      test("apply") {
        ...
        test("fromQueue") {
          for {
            ...
            _ <- (ZIO.foreach(0 to 100)(queue.offer(_)) *> queue.size
              .repeatUntil(_ == 0) *> queue.shutdown).forkDaemon
            stream = ZStream.fromQueue(queue)
            _ <- stream.foreach(value =>
              ref.update(_ + value)
            ) // will take from q until it is closed
            v <- ref.get
        ...
        test("fromFile") {
          ...
          for {
            _ <- (ZStream.fromPath(path) >>> decode)
...
object SimpleOperators extends ZIOSpecDefault {
  def spec =
    suite("SimpleOperators") {
      ...
      /** EXERCISE
        *
        * Use `runCollect` on the provided stream to extract out all the values
        * of the stream and collect them into a `Chunk`.
        */
      test("runCollect") {
        ...
        /** EXERCISE
          *
          * Insert a `dropWhile(_ < 3)` at the right place to drop the first two
          * elements of the stream.
          */
        test("dropWhile") {
          for {
        ...
        /** EXERCISE
          *
          * Insert a `filter(_ % 2 == 0)` at the right place to filter out all
          * the odd numbers.
          */
        test("filter") {
          for {
        ...
        test("forever") {
          for {
            // pull based - .the consumer (.take(5)) will terminate the stream (after pulling 5 times)
...
object RunningStreams extends ZIOSpecDefault {
  def spec =
    suite("RunningStreams") {
      ...
      test("runHead") {
        val stream =
          ZStream("All work and no play makes Jack a dull boy").forever

        for {
          headOption <- stream.runHead
        } yield assertTrue(
          headOption == Some("All work and no play makes Jack a dull boy")
        )
      } +
        /** EXERCISE
          *
        ...
        test("foldZIO") {
          ...
          for {
            ...
            map <- questions
              // .mapZIO(q => ZIO.succeed(q) <*> Console.readLine)
              .runFoldZIO(SortedMap.empty[String, String]) {
                case (map, question) =>
                  // val answer = question
  ...
  def spec =
    suite("AdvancedConstructors") {
      ...
      test("unfold") {
        ...
        /** EXERCISE
          *
          * Use `ZStream.repeatZIOOption` to repeat `Console.readLine` until the
          * line "John" is read from the console.
          */
  ...
  def spec =
    suite("AdvancedOperators") {
      ...
      test("flatMap") {
        ...
        /** EXERCISE
          *
          * Insert a `.mapZIO` to print out each question using
          * Console.printLine and ask for a response using Console.readLine.
          *
          * like .evalMap in fs2
          */
        test("mapZIO") {
          ...
          for {
            ...
            assertTrue(
              lines == Vector("What is your name?\n", "What is your age?\n")
            )
        } @@ ignore +
        /** EXERCISE
          *
          * Use `mapAccum` to keep track of word counts, emitting pairs of words
          * and their current running counts. Hint: Use a `Map[String, Int]` as
          * the state type for your `mapAccum`.
          */
        test("mapAccum") {
          val stream = ZStream("blue", "red", "blue", "red")

          def aggregate(
              stream: Stream[Nothing, String]
          ): Stream[Nothing, (String, Int)] =
            stream.mapAccum[Map[String, Int], (String, Int)](Map.empty) {
              case (map, str) =>
                map.get(str) match {
                  case Some(count) =>
                    (map + (str -> (count + 1)), (str -> (count + 1)))
                  case None => (map + (str -> 1), (str -> 1))
                }
            }
  ...
  def spec =
    suite("BasicError") {
      ...
      test("fail") {
        for {
          value <- (ZStream
            .fail("Uh oh!"): Stream[String, Int]).runCollect.either
        } yield assertTrue(value == Left("Uh oh!"))
      } +
        /** EXERCISE
          *
          * Use `.catchAll` to catch the error and turn it into a singleton
          * stream.
          */
        test("catchAll") {
          for {
  ...
  def spec =
    suite("ChunkedStreams") {
      ...
      test("foreachChunk") {
        ...
        /** EXERCISE
          *
          * Provide a correct implementation of `chunked` that exposes the
          * chunks underlying a stream.
          */
        test("chunked") {
          val stream = ZStream.fromChunks(
            Chunk(1),
          ...
          def chunked[R, E, A](
            ...
            stream.mapChunks(c => Chunk(c)) // chunks
        ...
        test("unchunked") {
          val stream =
            ZStream(Chunk(1), Chunk(2), Chunk(3, 4), Chunk(5, 6, 7, 8, 9))

          def unchunked[R, E, A](
              stream: ZStream[R, E, Chunk[A]]
          ): ZStream[R, E, A] =
            stream.mapChunks(_.flatten) // flattenChunks
...
object Graduation extends zio.ZIOAppDefault {
  def wordCount(file: String): IO[Throwable, Map[String, Int]] =
    ZStream
      ...
      .mapConcat(_.split("[^\\w]+"))
      .filterNot(_.isEmpty)
      .runFold(Map.empty[String, Int]) {
        _.updatedWith(_)(x => Some(x.fold(1)(_ + 1)))
      } 

      // .groupByKey(Predef.identity) { (word, stream) => 
      //   ZStream.fromZIO(stream.runCount.map(word -> _.toInt))
      // }
      // .runCollect
      // .map(_.toMap)
  ...
  def run =
    getArgs.map(_.headOption).flatMap {
      case None =>
        Console
          .printLine("You need to specify a file to word count")
          .!
          .as(ExitCode.failure)
      case Some(file) =>
        (for {
